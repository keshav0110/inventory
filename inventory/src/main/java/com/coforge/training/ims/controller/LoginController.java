...
public class LoginController {
	...
	public String viewRegisterPage(Model model) {
		...
		return "register";// It consists of model(dealer object) + view(register.jsp)
	}
	
	// Post Request --> Controller ---> Service--> DealerRepository--> JPA Repository --> Database
	...
	public ModelAndView loginDealer(HttpServletRequest req,@ModelAttribute("dealer") Dealer dealer) {
        ...
        String s2 = st.nextToken(); //email id without domain
         ...
         Optional<Dealer> d=lservice.findByEmail(email);//Fetch record/object from 2 tables(Dealer & Address) Based on Email id.
